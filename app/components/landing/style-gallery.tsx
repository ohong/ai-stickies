    ...
    <section id="styles" className="py-16 md:py-24 bg-white">
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-10 md:mb-16">
          ...
          <p className="mt-3 md:mt-4 text-muted-foreground max-w-lg mx-auto text-sm md:text-lg text-pretty">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-6">
          {styles.map((style) => (
            <div key={style.name}>
              <div className={`aspect-[4/5] rounded-xl md:rounded-2xl ${style.color} border ${style.borderColor} flex flex-col items-center justify-center p-3 md:p-4`}>
                <div className="size-16 md:size-24 rounded-full bg-white shadow-sm flex items-center justify-center mb-3 md:mb-4 overflow-hidden">
                  <Image
                    src={style.image}
                    alt={`${style.name} style example`}
                    width={96}
                    height={96}
                    className="object-cover size-16 md:size-24"
                  />
                </div>
        ...
        <div className="mt-16 md:mt-24 text-center">
          <h3 className="text-xl md:text-2xl font-bold text-foreground mb-6 md:mb-8 text-balance">Generated Examples</h3>
          <div className="grid grid-cols-3 md:flex md:justify-center gap-3 md:gap-4 max-w-sm md:max-w-none mx-auto">
            {sampleStickers.map((sticker, i) => (
              <div
                key={i}
                className="aspect-square md:size-32 rounded-xl md:rounded-2xl bg-white border border-gray-100 shadow-sm flex flex-col items-center justify-center overflow-hidden p-1.5 md:p-2"
              >
                <Image
                  src={sticker.image}
                  ...
                  width={80}
                  height={80}
                  className="object-contain mb-0.5 md:mb-1 size-14 md:size-20"
                />
                <span className="text-[10px] md:text-sm font-bold text-gray-600">{sticker.text}</span>
              </div>
            ))}
          </div>
          <p className="text-xs md:text-sm text-muted-foreground mt-4 md:mt-6 text-pretty">
