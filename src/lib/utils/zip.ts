...
import { PLATFORM_SPECS } from '@/src/constants/platform-specs'
...
/**
 * Create a platform-specific ZIP archive
 * Includes platform-appropriate naming and README
 */
export async function createPlatformZip(
  platform: Platform,
  stickers: Array<{ buffer: Buffer; filename: string; sequenceNumber: number }>,
  packName?: string
): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    for (const sticker of stickers) {
      archive.append(sticker.buffer, { name: sticker.filename })
    }

    const readme = generatePlatformReadme(platform, spec, stickers.length, packName)
    archive.append(readme, { name: 'README.txt' })
...
function generatePlatformReadme(
  platform: Platform,
  spec: (typeof PLATFORM_SPECS)[Platform],
  stickerCount: number,
  packName?: string
): string {
  const name = packName ?? 'Sticker Pack'
  const guides: Record<Platform, string> = {
    line: `How to Submit to LINE Creators Market:
1. Go to https://creator.line.me/
2. Create a LINE Creators Market account
3. Click "New Submission" > "Stickers"
4. Upload sticker files from this archive
5. Fill in title, description, and pricing
6. Submit for review (1-7 business days)`,

    whatsapp: `How to Use with WhatsApp:
1. Install a sticker maker app (e.g., Sticker Maker for WhatsApp)
2. Create a new sticker pack
3. Import the WebP files from this archive
4. Add the pack to WhatsApp
5. Minimum 3 stickers required per pack`,

    telegram: `How to Use with Telegram:
1. Open @Stickers bot in Telegram
2. Send /newpack to create a new pack
3. Follow the bot instructions
4. Upload each WebP file when prompted
5. Send /publish to make the pack available`,

    imessage: `How to Use with iMessage:
1. Open Xcode and create a new Sticker Pack Application
2. Drag the PNG files into the Sticker Pack folder
3. Build and run on your device
4. Or use a third-party sticker maker app`,
  }

  return `${name} - ${spec.name} Export
${'='.repeat(name.length + spec.name.length + 10)}

This archive contains ${stickerCount} stickers formatted for ${spec.name}.

Specifications:
- Format: ${spec.sticker.format.toUpperCase()}
- Dimensions: ${spec.sticker.width}x${spec.sticker.height} pixels
- Max file size: ${spec.sticker.maxSizeKB}KB per sticker
- Stickers per pack: ${spec.pack.minStickers}-${spec.pack.maxStickers}
